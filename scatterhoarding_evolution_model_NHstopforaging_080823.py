...
def reset(): #restarting entire simluation; first generation of first run only
    ...
    state2 = (pop_size)*[-1] # repeated pop_size times #state of all animals is "hungry"
    ...
    pred_risk = (pop_size)*[pred_risk_pop] #+ np.multiply(pred_risk_pop,storage_effort)
    # predation is same for all animals- added 8-8-23
...
def reset4(): #any first generation after first run
    ...
    season['Bout'] = 0
    season['Generation'] = 1
    state2 = (pop_size)*[-1] # state of all animals begins as hungry
    hoarder_id = [*range(0,pop_size)]
    ...
    for i in storage_effort_nh:
        storage_effort.append(i)
    pred_risk = (pop_size)*[pred_risk_pop] #+ np.multiply(pred_risk_pop,storage_effort)
    ### ^^ predation risk is now the same for all animals- added 8-8-23
    hoarder_values = {'state':state2, 'id':hoarder_id, 'stored food':hoarder_stored, 
    'predation risk': pred_risk, 'storage effort':storage_effort} 
    public_food_harv = 0
    foragers = 0
...
pop_size = 20 # number of animals in the population, always 20
days_tot = 100 # number of days in the season, always 100
runs_ = 450 #500 total for each combination of parameters

#Below variables are manipulated prior to running a specific variation of the model
pred_risk_pop = 0.0001 #population background risk
SH_pilfer_ability = 1 #1 = hoarders can pilfer, 0 = hoarders cannot pilfer
hoarders_num = 10 #number of animals with storage effort of 1 (i.e., hoarders); 1, 10, or 19
#pilf_for_eff = 0.5*pub_for_eff
...
pub_for_eff = [] #used to calculate detection probability, defined in function to run all code
...
state2 = (pop_size)*[-1] # state is hungry for all animals
...
def fun(hoarders_num, days_perish, mast_crop, for_eff):
    ...
    Added 8-8-23: If nonhoarders are satiated, they do not forage.
    ...
    if forager_id < pop_size: #if the forager is in the population 
        if hoarder_values['state'][foragers['hoarder'][forager_id]] < 1: #if state of forager identified by forager_id is alive
            if season['Bout'] == 1: #if it's the first bout of the day
                if foragers['foraged'][forager_id] == 0:
                    state() #determine state based on perishable and stored food
            if hoarder_values['storage effort'][foragers['hoarder'][forager_id]] == 0 and hoarder_values['state'][foragers['hoarder'][forager_id]] == 0: #if the nonhoarder is satiated
                    #print('The forager is a nonhoarder and has already eaten today')
                    #print('state:',hoarder_values['state'][foragers['hoarder'][forager_id]],'do they hoard?',hoarder_values['storage effort'][foragers['hoarder'][forager_id]])
                    #print(foragers)
                   # print('the nonhoarder is not hungry')
                   # print(hoarder_values)
               ...
               # print('state:',hoarder_values['state'][foragers['hoarder'][forager_id]],'do they hoard?',hoarder_values['storage effort'][foragers['hoarder'][forager_id]])
               # print('not a satiated NH')
               # print(hoarder_values)
                # otherwise; continue on as originally coded (added 8-8-23)
                if foragers['foraged'][forager_id] == 0: #and if they HAVEN'T foraged yet during this bout
                    foragers['foraged'][forager_id] += 1 #then define them as foraged,
...
def harvest_item(): #determine which item is harvested
    ...
    #global public_total
    global total_stored
    global public_food # changed from public_total on 8-17-23
    ...
    if other_SH > 0: #if any seeds are stored 
        other_SH_prop = other_SH / (other_SH + public_food)
        public_prop = public_food / (other_SH + public_food)
        ...
        if public_food > 0:
            ...
            predation() #no hoarded or public food available, end foraging
    food_fate()
...
def food_fate():
    ...
    global public_food_harv
    global harv_foodtype
    global public_food #changed from public_total 8-17-23
    ...
    if harv_foodtype == [0]: #if harvested food is public
        public_food = mast_crop - public_food_harv #public food currently available
        if public_food > 0: #if there is public food available
...
def predation():
    ...
    # on the last bout of the day, any starving animals will die after their attempt to forage
        if hoarder_values['state'][foragers['hoarder'][forager_id]] == -1: #and if forager is starving on the last bout of the day
            ...
            season['Generation'], #generation number
            ...
            hoarders_num, pred_risk_pop, detection_prob, food_avail, total_stored, pub_for_eff, pilf_for_eff,
        ...
        if hoarder_values['state'][foragers['hoarder'][forager_id]] < 995: # if the forager is alive (starving doesn't matter)
            ...
            if pred_rand < hoarder_values['predation risk'][foragers['hoarder'][forager_id]]: #if random number is less than forager's predation risk
                ...
                hoarder_values['storage effort'][foragers['hoarder'][forager_id]], #hoarder or nonhoarder
                season['Day'], hoarder_values['state'][foragers['hoarder'][forager_id]], #day of event, event type
                0, 1, 1, hoarders_num, pred_risk_pop, detection_prob, food_avail, total_stored, pub_for_eff, pilf_for_eff,#censor_starve, censor_pred, censor_dead
                days_perish, mast_crop, SH_pilfer_ability]
...
def reset2(): #all generations after the first, within a run
    ...
    season['Bout'] = 0    
    season['Generation'] += 1
    state2 = (pop_size)*[-1] #animals start as hungry
    hoarder_id = [*range(0,pop_size)]
    hoarder_stored = (pop_size)*[0]
    pheno = copy.deepcopy(phenotype)
    pred_risk = (pop_size)*[pred_risk_pop] #+ np.multiply(pred_risk_pop,pheno)
    # predation risk is same for all animals- added 8-8-23
    #pred_risk = pred_risk.tolist() # commented out 8-8-23
    #pred_risk = pred_risk # commented out 8-8-23
    hoarder_values = {'state':state2, 'id':hoarder_id, 'stored food':hoarder_stored, 
...
def write():
    ...
    input_variable = [season['Run'],season['Generation'], results['hoarder'], results['nonhoarder'],
        results['dead'],hoarders_num, pred_risk_pop, pub_for_eff,
        mast_crop, SH_pilfer_ability]
